#include <agile_grasp/grasp_localizer_suction.h>


GraspLocalizerSuction::GraspLocalizerSuction(ros::NodeHandle& node, const std::string& cloud_topic,
  ...
  num_clouds_received_(0), size_left_(0)
{
  ...
  if (cloud_type == CLOUD_SIZED)
		cloud_sub_ = node.subscribe(cloud_topic, 1, &GraspLocalizerSuction::cloud_sized_callback, this);
	else if (cloud_type == POINT_CLOUD_2)
		cloud_sub_ = node.subscribe(cloud_topic, 1, &GraspLocalizerSuction::cloud_callback, this);
...
void GraspLocalizerSuction::cloud_callback(const sensor_msgs::PointCloud2ConstPtr& msg)
{
...
void GraspLocalizerSuction::cloud_sized_callback(const agile_grasp::CloudSized& msg)
{
  // get point cloud from topic
...
void GraspLocalizerSuction::localizeGrasps()
{
  ros::Rate rate(1);
...
agile_grasp::Grasps GraspLocalizerSuction::createGraspsMsg(const std::vector<GraspHypothesis>& hands)
{
...
agile_grasp::Grasp GraspLocalizerSuction::createGraspMsg(const GraspHypothesis& hand)
{
...
agile_grasp::Grasps GraspLocalizerSuction::createGraspsMsgFromHands(const std::vector<Handle>& handles)
{
  agile_grasp::Grasps msg;  
...
agile_grasp::Grasps GraspLocalizerSuction::createGraspsMsg(const std::vector<Handle>& handles)
{
  agile_grasp::Grasps msg;  
  for (int i = 0; i < handles.size(); i++)
...
agile_grasp::Grasp GraspLocalizerSuction::createGraspMsg(const Handle& handle)
{
  agile_grasp::Grasp msg;
