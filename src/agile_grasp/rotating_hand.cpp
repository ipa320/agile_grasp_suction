...
RotatingHand::RotatingHand(const Eigen::VectorXd& camera_origin_left,
	...
	int cam_source, int num_orientations) :
...
void RotatingHand::transformPoints(const Eigen::Matrix3Xd& points,
		const Eigen::Vector3d& normal, const Eigen::Vector3d& axis,
		const Eigen::Matrix3Xd& normals,
		const Eigen::VectorXi& points_cam_source, double hand_height,
		bool crop_points_accordingto_Hight) {
	// rotate points into hand frame
	hand_axis_ = axis;
	frame_ << normal, normal.cross(axis), axis; // in the paper the frame is defined as normal, axis x normal, axis
	points_ = frame_.transpose() * points; // moving points to had frame
...
//  std::cout << " points_: " << points_.rows() << " x " << points_.cols() << std::endl;
	...
	normals_ = frame_.transpose() * normals; // moving normals to had frame
	points_cam_source_ = points_cam_source;
	if (crop_points_accordingto_Hight) {

		//std::cout << " Cropping points according to hand height: " << hand_height << "\n";
		//std::cout << " Hand_height = " << hand_height << "\n";

		// crop points by hand_height, I think he means the width of the fingers (not the thickens)
...
//    std::cout << "i: " << i << " " << points_.col(i).transpose() << "\n";
			...
			if (points_(2, i) > -1.0 * hand_height
					&& points_(2, i) < hand_height) {
	...
	bool use_antipodal)
{
	...
	for (int i = 0; i < n; i++)
	{
		...
		x << 1.0, 0.0, 0.0;
		y << 0.0, 1.0, 0.0;
//		Eigen::Vector3d approach = frame_ * rot.transpose() * y;/// old
		Eigen::Vector3d approach = frame_ * rot.transpose() * x;// approach to the normal

		// reject hand if it is not within 90 degrees of one of the camera sources
		if (approach.dot(cams_.col(0)) > 0 && approach.dot(cams_.col(1)) > 0)// this condition should be checked
		{
			//std::cout << "the hand is not within 90 degrees of one of the camera sources please advise...\n rotating_hand.cpp";
			continue;
		}

//		Eigen::Vector3d binormal = frame_ * rot.transpose() * x;// old
		Eigen::Vector3d binormal = frame_ * rot.transpose() * y;

		// calculate free hand placements
		finger_hand_.setPoints(points_rot);
		//finger_hand_.evaluateFingers(init_bite);
		finger_hand_.evaluateSuction();
		finger_hand_.evaluateHand();

		if (finger_hand_.getHand().sum() > 0)
		{
			// find deepest hand
			//finger_hand_.deepenHand(init_bite, finger_hand_.getHandDepth());// old
			finger_hand_.deepenHand(0.001, 0.01);//new 1mm to 10mm
			...
			if (use_antipodal && false) // if we need to evaluate whether the grasp is antipodal
			{
